...
function TheatreOwnerDashboard() {
  ...
  const [showForm, setShowForm] = useState({ movie: '', showTime: '', price: '' });
  ...
  const fetchAll = useCallback(async () => {
    setLoading(true);
    try {
      const [theatreRes, showsRes, bookingsRes, mealsRes, revenueRes, moviesRes, pointsRes] = await Promise.all([
      ...
      if (e.response?.status === 404) alert('No theatre assigned to your account. Contact admin.');
  ...
  const handleAddShow = async (e) => {
    e.preventDefault();
    try {
      await API.post('/theatre-owner/shows', showForm);
      setShowShowForm(false);
      setShowForm({ movie: '', showTime: '', price: '' });
      const res = await API.get('/theatre-owner/shows');
      setShows(res.data);
    } catch (err) { alert(err.response?.data?.message || 'Failed to add show'); }
  };

  const handleDeleteShow = async (id) => {
    if (!window.confirm('Delete this show?')) return;
    try {
      await API.delete(`/theatre-owner/shows/${id}`);
  ...
  const handleMealSubmit = async (e) => {
    e.preventDefault();
    try {
      if (editingMeal) { await API.put(`/theatre-owner/meals/${editingMeal._id}`, mealForm); }
      else { await API.post('/theatre-owner/meals', mealForm); }
      setShowMealForm(false); setEditingMeal(null);
      setMealForm({ name: '', description: '', price: '', category: 'snack', image: '', isAvailable: true });
      const res = await API.get('/theatre-owner/meals');
      setMeals(res.data);
    } catch (err) { alert(err.response?.data?.message || 'Failed to save meal'); }
  };

  const handleDeleteMeal = async (id) => {
    if (!window.confirm('Delete this meal?')) return;
    try { await API.delete(`/theatre-owner/meals/${id}`); setMeals(prev => prev.filter(m => m._id !== id)); }
  ...
  const handleRedeemPoints = async () => {
    ...
    if (!pts || pts < 100) return alert('Minimum 100 points to redeem');
    try {
      const res = await API.post('/loyalty/redeem', { pointsToRedeem: pts });
      alert(res.data.message);
      setRedeemPoints('');
      const p = await API.get('/loyalty/points');
      setMyPoints(p.data);
    } catch (err) { alert(err.response?.data?.message || 'Redemption failed'); }
  };
  ...
  return (
    <>
      <Navbar />
      <div style={{ background: colors.bg, minHeight: '100vh', color: colors.text, padding: '30px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px' }}>
          <div>
            <h1 style={{ color: '#ff004f', fontSize: '32px', margin: 0, fontWeight: '900' }}>🎭 Theatre Owner Dashboard</h1>
            <p style={{ color: '#64748b', margin: '5px 0 0', fontSize: '14px' }}>{theatre?.name || 'My Theatre'}</p>
          </div>
        ...
        <div style={{ display: 'flex', gap: '8px', marginBottom: '30px', flexWrap: 'wrap' }}>
          {TABS.map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)} style={{ padding: '9px 18px', borderRadius: '10px', border: activeTab === tab ? 'none' : '1px solid rgba(255,255,255,0.06)', cursor: 'pointer', fontWeight: '700', fontSize: '13px', background: activeTab === tab ? '#ff004f' : '#111827', color: activeTab === tab ? 'white' : '#94a3b8' }}>{tab}</button>
          ))}
        </div>
        ...
        {activeTab === 'Overview' && (
          <div>
            {theatre && (
              <div style={{ background: '#111827', border: '1px solid rgba(255,0,79,0.2)', borderRadius: '20px', padding: '28px', marginBottom: '24px' }}>
                <h2 style={{ margin: '0 0 16px', color: 'white', fontWeight: '800' }}>🏛️ {theatre.name}</h2>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(180px,1fr))', gap: '16px' }}>
                  <InfoItem label="City" value={theatre.city || '—'} />
                  <InfoItem label="Screens" value={theatre.screens || '—'} />
                  <InfoItem label="Your Share" value={`${100 - (theatre.commissionRate || 10)}%`} />
                  <InfoItem label="Admin Commission" value={`${theatre.commissionRate || 10}%`} />
                </div>
              </div>
            )}
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(190px,1fr))', gap: '20px', marginBottom: '24px' }}>
              <StatCard label="Total Shows" value={shows.length} icon="🎭" color="#6366f1" />
              <StatCard label="Confirmed Bookings" value={confirmed.length} icon="🎟️" color="#10b981" />
              <StatCard label="Cancelled" value={cancelled.length} icon="✕" color="#ef4444" />
            ...
            {myPoints && (
              <div style={{ background: 'linear-gradient(135deg,rgba(99,102,241,0.15),rgba(139,92,246,0.1))', border: '1px solid rgba(99,102,241,0.3)', borderRadius: '20px', padding: '24px' }}>
                <h3 style={{ color: '#818cf8', margin: '0 0 8px', fontWeight: '800' }}>🌟 My Loyalty Points: {myPoints.loyaltyPoints}</h3>
                <p style={{ color: '#64748b', margin: '0 0 16px', fontSize: '13px' }}>Worth ₹{myPoints.pointsValue} · 100 pts = ₹50 off</p>
                <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                  <input type="number" step="100" min="100" placeholder="Points to redeem (min 100)" value={redeemPoints} onChange={e => setRedeemPoints(e.target.value)} style={{ ...inputStyle, maxWidth: '260px' }} />
                  <button onClick={handleRedeemPoints} style={{ ...addBtnStyle, background: '#6366f1' }}>Redeem</button>
                </div>
              </div>
            )}
          </div>
        ...
        {activeTab === 'Shows' && (
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
              <h2 style={{ margin: 0 }}>My Shows ({shows.length})</h2>
              <button onClick={() => setShowShowForm(p => !p)} style={addBtnStyle}>+ Add Show</button>
            </div>
            {showShowForm && (
              <div style={{ background: '#111827', border: '1px solid rgba(255,0,79,0.2)', borderRadius: '16px', padding: '24px', marginBottom: '24px' }}>
                <h3 style={{ margin: '0 0 16px', color: 'white' }}>Add New Show</h3>
                <form onSubmit={handleAddShow}>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(200px,1fr))', gap: '14px' }}>
                    <select required value={showForm.movie} onChange={e => setShowForm(p => ({ ...p, movie: e.target.value }))} style={inputStyle}>
                      <option value="">-- Select Movie --</option>
                      {movies.map(m => <option key={m._id} value={m._id}>{m.title}</option>)}
                    </select>
                    <input required placeholder="Show Time (e.g. 25 Jul 2025, 7:00 PM)" value={showForm.showTime} onChange={e => setShowForm(p => ({ ...p, showTime: e.target.value }))} style={inputStyle} />
                    <input required type="number" placeholder="Ticket Price (₹)" value={showForm.price} onChange={e => setShowForm(p => ({ ...p, price: e.target.value }))} style={inputStyle} />
                  </div>
                  <div style={{ display: 'flex', gap: '10px', marginTop: '16px' }}>
                    <button type="submit" style={addBtnStyle}>Add Show</button>
                    <button type="button" onClick={() => setShowShowForm(false)} style={cancelBtnStyle}>Cancel</button>
                  </div>
                </form>
              </div>
            )}
            {shows.length === 0 ? <EmptyState message="No shows yet. Add your first show!" /> : (
                ...
                {shows.map(show => {
                  ...
                  return (
                    <div key={show._id} style={cardStyle}>
                      <h3 style={{ margin: '0 0 8px', color: '#ff004f', fontWeight: '800' }}>{show.movie?.title || 'Unknown'}</h3>
                      ...
                      <div style={{ margin: '10px 0 4px', display: 'flex', justifyContent: 'space-between' }}>
                        ...
                        <span style={{ color: col, fontSize: '12px', fontWeight: '700' }}>{booked}/{total} ({pct}%)</span>
                      </div>
                      ...
                      <button onClick={() => handleDeleteShow(show._id)} style={{ ...deleteBtnStyle, width: '100%', marginTop: '12px' }}>Delete Show</button>
                    </div>
                  );
                })}
              </div>
        ...
        {activeTab === 'Bookings' && (
          <div>
            <h2 style={{ marginBottom: '20px' }}>My Bookings ({bookings.length})</h2>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '14px' }}>
                  ...
                  <tbody>{bookings.map(b => (
                    <tr key={b._id} style={{ borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                      ...
                      <td style={tdStyle}><span style={{ background: b.status === 'CANCELLED' ? 'rgba(239,68,68,0.15)' : 'rgba(16,185,129,0.15)', border: `1px solid ${b.status === 'CANCELLED' ? 'rgba(239,68,68,0.4)' : 'rgba(16,185,129,0.4)'}`, color: b.status === 'CANCELLED' ? '#ef4444' : '#10b981', padding: '3px 10px', borderRadius: '20px', fontSize: '11px', fontWeight: '700' }}>{b.status}</span></td>
        ...
        {activeTab === 'Meals' && (
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
              <h2 style={{ margin: 0 }}>Meal Menu ({meals.length})</h2>
            ...
            {showMealForm && (
              <div style={{ background: '#111827', border: '1px solid rgba(255,0,79,0.2)', borderRadius: '16px', padding: '24px', marginBottom: '24px' }}>
                <h3 style={{ margin: '0 0 16px', color: 'white' }}>{editingMeal ? 'Edit Meal' : 'Add New Meal'}</h3>
                <form onSubmit={handleMealSubmit}>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(200px,1fr))', gap: '14px' }}>
                    ...
                    <select value={mealForm.category} onChange={e => setMealForm(p => ({ ...p, category: e.target.value }))} style={inputStyle}>
                      <option value="snack">Snack</option><option value="drink">Drink</option><option value="combo">Combo</option><option value="dessert">Dessert</option>
                    </select>
                  ...
                  <div style={{ display: 'flex', gap: '10px', marginTop: '16px' }}>
                    <button type="submit" style={addBtnStyle}>{editingMeal ? 'Update' : 'Add Meal'}</button>
                    <button type="button" onClick={() => { setShowMealForm(false); setEditingMeal(null); }} style={cancelBtnStyle}>Cancel</button>
                  </div>
                </form>
              </div>
            )}
            {meals.length === 0 ? <EmptyState message="No meals yet." /> : (
        ...
        {activeTab === 'Revenue' && (
          <div>
            <h2 style={{ marginBottom: '24px' }}>Revenue Breakdown</h2>
            {!revenue ? <EmptyState message="Loading..." /> : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
                ...
                <div style={{ background: '#111827', borderRadius: '20px', padding: '28px', border: '1px solid rgba(255,255,255,0.06)' }}>
                  ...
                  {[
                    { label: `Your Share (${100-(revenue.theatre?.commissionRate||10)}%)`, pct: 100-(revenue.theatre?.commissionRate||10), val: revenue.computed?.ownerRevenue||0, color: '#10b981' },
                    { label: `Admin Commission (${revenue.theatre?.commissionRate||10}%)`, pct: revenue.theatre?.commissionRate||10, val: revenue.computed?.adminRevenue||0, color: '#6366f1' }
                  ].map(item => (
                    <div key={item.label} style={{ display: 'flex', gap: '12px', alignItems: 'center', marginBottom: '14px' }}>
                      <span style={{ color: '#94a3b8', fontSize: '13px', width: '200px' }}>{item.label}</span>
                      <div style={{ flex: 1, height: '12px', background: '#1e293b', borderRadius: '6px', overflow: 'hidden' }}>
                        <div style={{ height: '100%', width: item.pct+'%', background: item.color, borderRadius: '6px' }} />
                      </div>
                      <span style={{ color: item.color, fontWeight: '700', fontSize: '14px', width: '80px', textAlign: 'right' }}>₹{item.val.toLocaleString()}</span>
                    </div>
                  ))}
                </div>
                {(revenue.movieBreakdown||[]).length > 0 && (
                  <div style={{ background: '#111827', borderRadius: '20px', padding: '28px', border: '1px solid rgba(255,255,255,0.06)' }}>
                    ...
                    {revenue.movieBreakdown.map((item, i) => {
                      ...
                      const cols = ['#ff004f','#6366f1','#0ea5e9','#10b981','#f59e0b'];
                      return (
                        <div key={i} style={{ marginBottom: '16px' }}>
                          ...
                          <div style={{ height: '8px', background: '#1e293b', borderRadius: '4px', overflow: 'hidden' }}>
                            <div style={{ height: '100%', width: pct+'%', background: cols[i%cols.length], borderRadius: '4px' }} />
                          </div>
                        </div>
                      );
                    })}
