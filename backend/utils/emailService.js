...
const sendTicketEmail = async ({ to, booking, movie, theatre, show }) => {
  ...
  <body>
    <div class="wrapper">
      ...
      <div class="ticket">
        ...
        <div class="qr-note">
          <p>📱 Show your Booking ID at the theatre entrance or download your PDF ticket from the app</p>
        </div>
      </div>
      <div class="footer">
  ...
  // ── STEP 1: Send HTML email immediately (guaranteed delivery) ──
  ...
  // ── STEP 2: Try to send PDF attachment as a separate follow-up email ──
  // This runs non-blocking after the main email is already sent
  try {
    ...
    await transporter.sendMail({
      from: `"CineVerse 🎬" <${process.env.EMAIL_USER}>`,
      to,
      subject: `📄 Your PDF Ticket — ${movie?.title || 'Your Movie'} | ${seatList}`,
      html: `<div style="font-family:sans-serif;background:#0a0f1e;color:white;padding:24px;border-radius:12px;">
        <h2 style="color:#ff004f">🎬 CineVerse — PDF Ticket</h2>
        <p>Your PDF ticket for <strong>${movie?.title}</strong> is attached below.</p>
        <p style="color:#64748b;font-size:13px;">Booking ID: ${booking.bookingId}</p>
      </div>`,
      attachments: [{
    ...
    console.error('PDF ticket email failed (HTML email was already sent):', pdfErr.message);
  }
};
